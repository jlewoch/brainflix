...
router
  .route("/:videoId")
  .get((req, res) => videoServices.videoIdFunc(req.params.videoId) ? res.status(200).json(videoServices.videoIdFunc(req.params.videoId)): res.status(404).send('Page Not Found'))
  .put((req, res) => {
    if (req.body.title && req.body.description && videoServices.videoIdFunc(req.params.videoId)) {
      videoServices.videoIdFunc(req.params.videoId).title = req.body.title;
      videoServices.videoIdFunc(req.params.videoId).description = req.body.description;
...
router.route("/:videoId/comments").post(function(req, res) {
  if (req.body.name && req.body.comment) {
    ...
    res.status(200).json(videoServices.videoIdFunc(req.params.videoId).comments);
  } else {
    res
      .status(400)
      .json(
...
router
  ...
  .put((req, res) => {
    if (req.body.comment && videoServices.videoIdFunc(req.params.videoId)) {
      videoServices.videoIdFunc(req.params.videoId).comments.find(
  ...
  .delete(function(req, res) {
    videos[videos.indexOf(videoServices.videoIdFunc(req.params.videoId))].comments = videos[
